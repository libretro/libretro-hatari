...
#else 
				RGBColor = RGB565(rr>>3, gg>>3, bb>>3) ;
				if (1)
#endif
				{
...
#else
	Screen_SetDrawFunctions(16, bDoubleLowRes);
#endif
...
#else 
        memset (sdlscrn.bitmap, 0, sdlscrn.stride * sdlscrn.h); 
#endif
}
...
#else 
	pPCScreenDest = (unsigned char *)&sdlscrn.bitmap[0]; 
#endif
#ifndef RETRO
	PCScreenBytesPerLine = sdlscrn->pitch;        /* Bytes per line */
#else 
	PCScreenBytesPerLine = sdlscrn.stride;
#endif
...
#else 
	pPCScreenDest += PCScreenOffsetY * PCScreenBytesPerLine + PCScreenOffsetX * 2;
#endif
