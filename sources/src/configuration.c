...
//DEB RETRO HACK
	...
	psHomeDir =(char *) Paths_GetHatariHome();
	psWorkingDir = (char *)Paths_GetWorkingDir();

#ifdef PS3PORT
 	strcpy( psWorkingDir,"/dev_hdd0/HOMEBREW/ST");
 	strcpy( psHomeDir,"/dev_hdd0/HOMEBREW/ST");
	...
	//DEB RETRO HACK
#ifdef RETRO
	strcpy(ConfigureParams.Log.sLogFileName, "./1log.txt"/*"stderr"*/);
	strcpy(ConfigureParams.Log.sTraceFileName, "./1trace.txt"/*"stderr"*/);
#else
...
#else
	ConfigureParams.Log.nTextLogLevel =  LOG_DEBUG;//LOG_INFO;//LOG_TODO;
	ConfigureParams.Log.nAlertDlgLogLevel = LOG_ERROR;
#endif
...
#else
//TODO support 832*576 for other than android poor phones
//DONE?
	ConfigureParams.Screen.nMaxWidth = retrow;
	ConfigureParams.Screen.nMaxHeight = retroh;
#endif	
...
#else
#ifdef WIN32PORT
sprintf(ConfigureParams.Rom.szTosImageFileName, "%srom", ".\\");
#else
sprintf(ConfigureParams.Rom.szTosImageFileName, "%srom", "./");
#endif
#endif
